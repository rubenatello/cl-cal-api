...
export async function handler() {
    ...
    try {
        ...
        // ✅ Filter only "City Group" events
        const cityGroupEvents = data.data.filter(event => 
            event.attributes.name.toLowerCase().includes("city group")
        );
        ...
        return {
            statusCode: 200,
            body: JSON.stringify(cityGroupEvents),
        };
    } catch (error) {
